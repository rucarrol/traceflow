...
class socket_handler:
    def __init__(self, daddr):
        ...
        if os_release == "Darwin":
            ...
            logging.debug('Detected Mac OS - Cannot support writing of raw IP packets, exiting')
            exit(1)
...
class socket_listener:
    def __init__(self, ip_daddr):
        ...
        try:
            self.icmp_listener = socket.socket(socket.AF_INET, socket.SOCK_RAW, socket.getprotobyname("icmp"))
        except PermissionError as e:
            print(e)
            print("Please run as root!")
            exit(1)
    ...
    def listener(self):
        ...
        while True:
            ...
            ip_id = traceflow.packet_decode.decode_ipv4_header(icmp_packet_ret['payload'])['ip_id']
            # Did we get a TTL Expired (11)?
            if icmp_packet_ret['type'] == 11:
                logging.debug('Got TTL Expired from %s with ip_id %s' % (curr_addr[0], ip_id))
                self.mutex.acquire()
    ...
    def get_packet_by_ipid(self, ipid: int) -> dict:
        ...
        for packet in self.icmp_packets.keys():
            icmp_packet = traceflow.packet_decode.decode_icmp(self.icmp_packets[packet]['payload'])
